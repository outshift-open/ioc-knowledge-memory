...
class InMemoryStorage:
    ...
    def delete_workspace(self, workspace_id: str) -> bool:
        with self._lock:
            if workspace_id in self._workspaces:
                users_to_delete = [
                    user_id for user_id, user in self._users.items() if user.workspace_id == workspace_id
                ]
                for user_id in users_to_delete:
                    del self._users[user_id]

                api_keys_to_delete = [
                    key_id for key_id, api_key in self._api_keys.items() if api_key.workspace_id == workspace_id
                ]
