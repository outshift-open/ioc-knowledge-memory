...
class SoftwareCreate(BaseModel):
    ...
    config: Optional[SoftwareConfig] = Field(default=None, description="Software-specific configuration")
    ...
    def validate_type(cls, v):
        if v not in [SoftwareType.REASONER.value, SoftwareType.KEP_ADAPTER.value]:
            raise ValueError(
                f'type must be either "{SoftwareType.REASONER.value}" or "{SoftwareType.KEP_ADAPTER.value}"'
            )
        return v
...
class SoftwareResponse(BaseModel):
    ...
    api_key_id: Optional[str] = Field(default=None, description="Associated API key UUID")
