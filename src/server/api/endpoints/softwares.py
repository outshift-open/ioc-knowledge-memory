...
def create_software(workspace_id: str, software_data: SoftwareCreate):
  ...
  try:
    ...
    raise HTTPException(
      ...
      detail=f"Failed to create software: {str(e)}"
    )
...
def list_softwares(workspace_id: str):
  ...
  try:
    ...
    softwares = [
      ...
      if sw.deleted_at is None and sw.workspace_id == workspace_id  # Only return non-deleted software in this workspace
    ]
    ...
    raise HTTPException(
      status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
      detail=f"Failed to list softwares: {str(e)}"
    )
...
def get_software(workspace_id: str, software_id: str):
  ...
  try:
    ...
    raise HTTPException(
      status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
      detail=f"Failed to retrieve software: {str(e)}"
    )
...
def update_software(workspace_id: str, software_id: str, software_data: SoftwareUpdate):
  ...
  try:
    ...
    if not success:
      raise HTTPException(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        detail="Failed to update software"
      )
    ...
    raise HTTPException(
      status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
      detail=f"Failed to update software: {str(e)}"
    )
...
def delete_software(workspace_id: str, software_id: str):
  ...
  try:
    ...
    if not success:
      raise HTTPException(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        detail="Failed to delete software"
      )
    ...
    raise HTTPException(
      status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
      detail=f"Failed to delete software: {str(e)}"
    )
